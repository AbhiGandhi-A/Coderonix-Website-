    ...
    useEffect(() => {
        ...
        if (socket) {
            const handleEventCreated = (newEvent) => {
                // Check if the event already exists in state to prevent duplicates from the creator's own socket event
                if (!events.some(event => event._id === newEvent._id)) {
    ...
    const createEvent = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // We rely on the socket event to update the state for all clients
                if (socket) {
    ...
    const joinEvent = async (eventId) => {
        try {
            ...
            if (response.ok) {
                ...
                // The socket listener will handle updating the state for all users, including the one who clicked the button
                if (socket) {
                                ...
                                <div className="event-details">
                                    ...
                                    {(event.type === 'meeting' || event.type === 'celebration') && (
                                        <button 
                                            ...
                                            className={`btn-join ${isUserJoined(event) ? 'joined' : ''}`}
                                            disabled={isUserJoined(event)}
                                        >
                                            {isUserJoined(event) ? 'Joined' : 'Join'}
                                        </button>
                                    )}
                                </div>
                            </div>
